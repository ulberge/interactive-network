...
import { dtype } from '../../js/conv/convArray';
import p5 from 'p5';
...
export default class SmartCollage {
  constructor(p, shape, networks) {
    // assumes p is blank
    this.p = p;
    this.shape = shape;
  ...
  update(notify=true) {
    if (this._dirtyBounds) {
      ...
      this.networks.forEach(network => network.run(dirty, this._dirtyBounds));
      if (notify) {
        this._notifyListeners({ networks: this.networks, dirtyBounds: [...this._dirtyBounds] });
      }
